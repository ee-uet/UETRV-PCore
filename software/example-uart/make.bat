...
python maketxt.py build/main.bin > build/imem.txt

riscv64-unknown-elf-objdump -S -s build/program.elf > build/program.dump

REM riscv64-unknown-elf-objcopy --input-target=binary --output-target=elf32-little build/bbl.bin build/bbl.elf

REM riscv64-unknown-elf-objdump -S -s build/bbl.elf > build/bbl.dump
